    ...
    public static void main(String[] args) throws IOException {
        ...
        try (
            ...
            while ((inputLine = in.readLine()) != null && !inputLine.equalsIgnoreCase("Bye")) { // check that the message received is not null or Bye
                System.out.println(inputLine); // print what the server types
                System.out.print(name + ": "); // prepare the turn for the client to enter a response
                ...
                if (outputLine != null) { // make sure that the input from the client is not a null
                    out.println(name + ": " + outputLine); // send the response to the server from the client
                } else {
                    break;
                }
            }
